	...
	for(i = 1, j=i; i < n; i++){
		...
		while(u->first->prev != NULL && u->first->prev->info.cod > u->first->info.cod){
			...
			if(u->first->prox == NULL){
				puts("B\n");
				u->first->prev->prev->prox = u->first;
				u->first->prev->prox = NULL;
				u->first->prev->prev = u->first;
				u->first->prox = u->first->prev;
				u->first->prev = atual.prev->prev;
				j--;
			}else {
				puts("C\n");
				u->first->prox->prev = u->first->prev;
				u->first->prev->prev->prox = u->first;
				u->first->prev = u->first->prev->prev;
				u->first->prox = u->first->prox->prev;
				aux = u->first;
				u->first = u->first->prox;
